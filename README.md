...
**Teams.** Invite the people you work with and see, per person, the computers they connected and which accounts their Claude Code and Codex CLIs use, every Claude Code and Codex session on those computers (live ones first, with their project, branch, models, tokens and API value), the projects they work on, and how close each of their accounts is to its limits; and, on the Analytics tab, how each of them works with Claude Code and Codex: prompts, lines changed, active time, sessions, tokens and what they are worth, per day and person, when they work, on what and with which models. Open any session as a chat, read it, and send the next message from the browser, whether it started in a terminal, an IDE or the Claude and Codex desktop apps; start new ones on any of your team's computers that allow it, and switch their CLI logins or start a 5-hour window on your own, all from the website, on a phone too. [More below](#teams).

**The small things.** Browser notifications when a limit resets, nears its end or runs out, with the threshold, quiet hours and muted accounts yours to pick in **Settings**. A countdown in the tab title so a pinned tab is enough. One-click "copy status" for pasting into chat. Rename accounts to whatever makes sense to you, from each account's **⋯** menu, which also refreshes, copies its status, mutes it or removes it (with **Undo**). **Ctrl+K** (⌘K on a Mac) searches every account and command. A refresh button on everything that can go stale (the Team page, your sessions, This machine, a chat's conversation), saying when it last read. Light and dark themes, or follow the system.
...
- **Sign in** with your Claude or ChatGPT account. This uses the same OAuth flow the official CLIs use, so the app can refresh its own tokens and keep polling for weeks. On the website you paste back the code Claude shows, or the address ChatGPT's sign-in ends on; the [desktop app](#desktop-app) picks up ChatGPT's by itself.
...
[Teams](#teams) add three things, all opt-in: a computer you connect sends its activity report (its sessions and project folders, git branches, models, times and token counts from the CLIs' session logs, and per day how many prompts, file changes, changed lines and commands there were, not what the sessions say or change); a computer that lets what sessions say reach the website also sends session titles, and a session's conversation (and each of its images) when someone who may read it asks for it, which the server keeps encrypted like provider tokens, compressed, for a week (images for three days: the computer sends them again when asked); and a remote session's prompt and output go through the server, which keeps them encrypted for 30 days.
...
- **It lives in the tray.** Closing the window keeps AI Cooldown running in the tray (the Dock on macOS), so scheduled hellos and notifications keep going on whichever page it was left, the Team page included. The tray icon carries a dot, green while every provider has room, amber when the best account is under 20%, red when every Claude or every Codex account is out, and its tooltip and menu say which account to use or how long to wait. Click it to show or hide the window; its menu also refreshes, adds an account, turns notifications on or off and quits. **Settings** can make closing the window quit instead.
- **It behaves like a Windows app.** On Windows the window has its own title bar, with the Dashboard and Team pages, a search box for every account and command, and a **Restart to update** button when an update is ready. It opens where you left it, at the size you left it, maximized if it was; shows up at once while it starts; has right-click menus to cut, copy and paste; a jump list (right-click its taskbar button: Refresh now, Add account, Team, Settings); a red badge on the taskbar button while every account of a provider is out; and, if you set one in **Settings**, a shortcut that shows or hides it from any app.
- **Keyboard first.** **Ctrl+K** searches accounts and commands, **Ctrl+R** or **F5** refreshes, **Ctrl+N** adds an account, **Ctrl+1** and **Ctrl+2** switch pages, **Ctrl+Shift+C** copies the status, **Ctrl+,** opens Settings, **Ctrl+Plus** and **Ctrl+Minus** zoom (remembered), **F11** goes full screen, **Ctrl+W** closes the window and **Ctrl+Q** quits. Press **?** for the list.
- **Notifications are on.** When a limit resets, crosses 90% or runs out, you get a system notification, even with the window closed; clicking it opens the window. **Settings**, Notifications, picks which of those you hear about, the share that counts as nearly used up, quiet hours and muted accounts, and sends a test.
- **Signing in to ChatGPT finishes by itself.** The desktop app answers on `localhost:1455`, where the ChatGPT sign-in sends your browser, so there is no address to copy back (unless `codex login` is using that port at the same moment).
- **It opens at login,** straight to the tray, once installed. Turn that off in **Settings**, General, or with **Open at login** in the tray icon's menu (the AI Cooldown menu on macOS).
- **It keeps itself up to date.** Installed on Windows or Linux, it checks this repository's releases every few hours, downloads a new version in the background (checked against the SHA-512 in the release's `latest.yml`), and installs it the next time the window is closed, then carries on in the tray; the title bar, **Settings**, About and the tray menu can also restart into it right away. On macOS it cannot replace itself without an Apple Developer ID signature, so it tells you when a new version is out and opens the download. Versions before 0.4 did not update themselves: install 0.4 once by hand.
...
- **Your data is in one folder:** `%LOCALAPPDATA%\AI Cooldown` on Windows, `~/Library/Application Support/AI Cooldown` on macOS, `~/.local/share/aicooldown` on Linux. `data` holds the saved CLI logins, schedules, the local database and its key; `window` holds the window's own storage (guest accounts, polling history, theme and notification choices) and `settings.json`, the app's own settings (where the window was, zoom, what closing it does, the shortcut). **Settings**, General, opens the folder. Guest accounts you added in a browser tab with the single-file versions before 0.3 stayed in that browser, so add them again or sign in.
...
- **Roles.** The owner renames or deletes the team, changes roles and can hand the team to someone else. Admins invite and remove members. Owners and admins see the members' details; everyone sees their own, and of anyone else what that person shares with them.
- **What owners and admins see of a member**: all of their work. Their connected computers (name, system, AI Cooldown version, online or when last seen, and whose accounts the Claude Code and Codex CLIs there are signed in with), every Claude Code and Codex session on them over the last 30 days with its title and conversation, the projects worked on there with tokens per day and model and what those tokens would cost at Anthropic's and OpenAI's API list prices, how many prompts, file changes, changed lines and commands each day had and when, the latest reading of each linked account's limits, and the remote sessions started on their computers from this team. They can continue a member's sessions where the computer allows remote sessions. Members are managed: their computers let what sessions say reach their teams while they are members, and the invite says so before they join. Never provider tokens.
- **What owners and admins show each other**: their computers and account limits, and of their projects and sessions only what each shares, under the team's name: **What you pick** (the default: nothing until you share something) or **Everything**.
- **Sharing with people you pick**: anyone, member or admin, shares one of their projects (by name, on all their computers) from the Projects tab, or one chat from its header, with the team's other owners and admins, with everyone in the team, or with the people they tick. Those see its sessions, and what they say where the computer lets that reach the website, and nothing else of the sharer's: not their computers (only a name where a shared session is), accounts or other projects. Sharing is for reading: sessions are started and continued by a computer's owner and the owners and admins of their teams only, in what they see. What someone does not share does not reach anyone else at all, not even that it exists: not its sessions, tokens, titles, conversations or remote sessions. Someone in two teams is seen by the owners and admins of both; a remote session started from one team stays out of the other's sight.
- **Six views**: Overview (who is on what, what is live now, totals, tokens per day, limits running out, busiest projects), Analytics (below), People (each with their latest sessions), Computers (each person's together, and which accounts are signed in where), Projects (each with who works on it where, and its latest sessions) and Sessions, over 7, 14 or 30 days. **New chat** starts a conversation on any computer you may use.
- **Analytics** shows how the people you see work with Claude Code and Codex, for everyone or one person: people active, sessions, prompts, lines changed, active time and API value, each next to the period before; a chart per day of tokens, API value, sessions, prompts, lines changed or active time, stacked by person; everyone side by side, sortable, with their share and days; when the work happens, by weekday and hour; the busiest projects split by person; the models used; and ratios (prompts per session, lines per prompt, cache reads, API value against what the linked accounts' plans cost). Prompts count what people typed, not the CLIs' own messages or the tasks sessions give their subagents; lines changed are the lines the CLIs' edits and patches added and removed; active time counts the quarter hours with any prompt, reply, command or change, not working hours. Those three come from computers running AI Cooldown 0.13 or later, and the tab says which computers do not report them yet. Members see their own; owners and admins see everyone's, of another owner or admin what they share.

### Connect a computer

In the desktop app, or any copy running on your computer, sign in and press **Connect this computer** under This machine. The computer gets a token of its own and checks in with the server that keeps your account every minute (every 15 seconds while it allows remote sessions or lets what sessions say reach the website, every 2 seconds while someone has one of its conversations open on the website): what it is, which accounts its CLIs use and whether they can run remote sessions, plus, when it changed, an activity report it reads every 2 minutes from the CLIs' session logs (`~/.claude/projects`, `~/.codex/sessions`): per project folder, with the home folder shown as `~`, the git branch, models, times and token counts, per day how many prompts were typed, files changed, lines added and removed (counted from the edits and patches the CLIs log) and shell commands run, and which quarter hours had any of it, and each session with where it ran (terminal, IDE, desktop app, `codex exec`), when it started and was last active, its models, tokens, subagents and the same counts. A conversation written into several logs (a resumed or forked session, a chat copied to another account) counts once. **Disconnect** stops it for good. Signing out there disconnects it until you sign in there again, and signing out other devices (or changing your password) disconnects every computer until then.
...
On the first page, **Your sessions** lists your own computers' live and latest Claude Code and Codex sessions, with a chip for each computer: open any of them as a chat, or start a **New chat** on one of your computers, without going to the Team page (**All sessions** goes to all of yours there). It shows once a computer is connected.

Open a session and it reads like a chat: what was said so far, then a box for the next message. Send it, and the computer runs it as the next turn of the same conversation (`claude -p --resume`, `codex exec resume`), streams the reply back, and the conversation carries on from there, on the computer too. It reads four ways: **Chat**; **Compact**, only what was asked and answered, for a quick look on a phone; **Verbose**, every step in full with its time; and **Images**, a gallery of the images pasted with prompts or returned by tools (screenshots, image files read), each opened large with the ones before and after a tap away. Images come from the computer as they come into view, one per request: PNG, JPEG, GIF and WebP up to 2 MB, checked to be the pictures they say they are, kept encrypted for three days (a hundred per computer) and deleted with the conversations when a computer turns private. It works for every conversation on the computer, whether it started in a terminal, an IDE, the Claude or Codex desktop apps, or here: the computer's owner can continue any of them, and the owners and admins of their teams can continue the ones they see when what sessions say reaches the website (a reply can tell what was said before; AI Cooldown 0.7 or later on the computer). A Claude Code conversation open in a program on that computer, such as a chat open in the Claude app or a terminal still in it, takes the message there, the way Claude Code sessions on one computer message each other (Claude Code 2.1.234 or later): it goes into that conversation and runs on that program's login (the Claude Code CLI need not be signed in for it), with that program's model and permissions, and the reply streams back here as it is written. So the level you send it with has to cover what the chat may do there without asking: any level for a chat in Plan mode, Edit files for Ask permissions, Accept edits or Don't ask, and Full access for Auto or Bypass permissions, or for a terminal, whose mode AI Cooldown can't see; the computer says so otherwise. The chat shows it as a message from AI Cooldown (and from whom, when it isn't the computer's owner); one that skips permission prompts asks at the computer before taking it, and **Stop** only stops following the reply, which goes on there. A conversation open in nothing continues through the Claude Code CLI, which has its own sign-in: the Claude app's does not carry over, so to continue a Claude app chat that is not open, sign the CLI in once on that computer (`claude auth login`, with the same account; This machine shows the command). One open in a program that takes no messages (Codex, or an older Claude Code) is continued there, or closed there first. The message goes with what the conversation last used (or what you pick: read only, edit files, full access, within what the computer allows; the model), and **Stop** ends a reply that runs too long. While a conversation is open its computer checks in every 2 seconds, so a message starts within a few seconds, and a finished reply notifies you when the page is in the background (with notifications on). Everything here works from a phone.
