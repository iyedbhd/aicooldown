...
What sessions say stays on the computer unless its owner shares it, under **Session content** in This machine, which only that computer decides: **Private** (the default), **Only me** (you read your own sessions from the website, your teams see them without titles), or **Me and my teams** (the owners and admins of your teams read them too). It asks before sharing. A sharing computer sends each session's title (Claude Code's custom title or summary, Codex's thread name, or else its first prompt), and when someone it shares with opens a session, it reads the conversation from the CLI's log and sends it on its next check-in: prompts, replies, the commands run and what they printed, the latest 1,500 entries. The server keeps it encrypted for a week, and deletes every conversation from a computer as soon as it shares with fewer people.

### Chat with any session

Open a session and it reads like a chat: what was said so far, then a box for the next message. Send it, and the computer runs it as the next turn of the same conversation (`claude -p --resume`, `codex exec resume`), streams the reply back, and the conversation carries on from there, on the computer too. It works for every conversation on the computer, whether it started in a terminal, an IDE, the Claude or Codex desktop apps, or here: the computer's owner can continue any of them, and the owners and admins of their teams can when the computer shares session content with them (a reply can tell what was said before). The message goes with what the conversation last used (or what you pick: read only, edit files, full access, within what the computer allows; the model), and **Stop** ends a reply that runs too long. While a conversation is open its computer checks in every 2 seconds, so a message starts within a few seconds, and a finished reply notifies you when the page is in the background (with notifications on). Everything here works from a phone.

### Manage your computers

On the Computers tab, each computer says whether Claude Code and Codex can run remote sessions there: ready, not signed in, or not installed. On your own computers, **Manage logins and hellos** does from anywhere what This machine does on the computer: switch the login a CLI uses to one of the logins saved there, save the one it uses now, say hello to start a login's 5-hour window, and schedule a hello (at the next reset, after every reset, or at a time) or cancel one. The computer does it on its next check-in, within seconds while the page is open, and the card shows how each went. Only a computer's owner sends these; deleting a saved login stays on the computer.
...
src/lib/server/{teams,devices,runs,transcripts,commands,workspace}.ts
