...
**The small things.** Browser notifications when a limit resets, nears its end or runs out, with the threshold, quiet hours and muted accounts yours to pick in **Settings**. A countdown in the tab title so a pinned tab is enough. One-click "copy status" for pasting into chat. Rename accounts to whatever makes sense to you, from each account's **⋯** menu, which also refreshes, copies its status, mutes it or removes it (with **Undo**). **Ctrl+K** (⌘K on a Mac) searches every account and command. Light and dark themes, or follow the system.
